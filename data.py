...
def signUpFunc(name, email, password, ba, sa):
    ...
    if user:
        ...
        try:
            mycursor.execute(
                "insert into Users values ('%s', '%s', '%s','%s', '%s', '%s', '%s') " % userInfo)
            mydb.commit()
...
def getUserBasket(id):
    try:
        mycursor.execute(
            "select * from Book_Basket where Basket_ID = '%s'" % id)
...
def findAmountInCollection(isbn):
    try:
        mycursor.execute(
            "select Amount from Book_BookStore where ISBN = '%s' " % isbn)
...
def checkBookInBasket(basketId, isbn):
    try:
        mycursor.execute(
            "select * from Book_Basket where Basket_ID = '%s' and ISBN = '%s'" % (basketId, isbn))
        book = mycursor.fetchall()
        if book:
            return book
        else:
...
def addToBasket(basketId, isbn, amount):
    ...
    if book:
        ...
        try:
            mycursor.execute(
                "Update Book_Basket set amount = amount + '%s' where Basket_ID = '%s' and ISBN = '%s'" % (amount, basketId, isbn))
            mydb.commit()
        ...
        # add book to basket
        try:
            mycursor.execute("insert into Book_Basket values ('%s' ,'%s' ,'%s')" % (
                basketId, isbn, amount))
            mydb.commit()
...
def removeFromBasket(basketId, isbn):
    try:
        mycursor.execute(
            "delete from Book_basket where Basket_ID = '%s' and ISBN = '%s' " % (basketId, isbn))
        mydb.commit()
...
def buyBooks(id, basketID, sa, ba):
    ...
    for x in basket:
        ...
        if(checkCollection(x[1], x[2])):
            ...
            addBankTransfer(ref, month, year,
                            bookInfo[0][2], bankId, storeId, amount)
            Removed.append(x)
    for x in Removed:
...
def addToBank(bankId, referenceNo):
    ...
    try:
        mycursor.execute("insert into Bank values ('%s', '%s','%s', '%s','%s', '%s') ;" % (
            bankId, referenceNo, name, address, email, number))
        mydb.commit()
...
def addToPubBank(bankId, publisherId, referenceNo):
    try:
        mycursor.execute("insert into Publisher_Bank values ('%s', '%s' ,'%s') ;" % (
            publisherId, bankId, referenceNo))
        mydb.commit()
...
def addToBookBank(storeId, bankId, referenceNo, amount, month, year):
    try:
        mycursor.execute("insert into Bookstore_Bank values ('%s', '%s', '%s' ,%d ,%d ,%d) ;" % (
            storeId, bankId, referenceNo, amount, month, year))
        mydb.commit()
...
def reduceBookStoreAmount(isbn, amount):
    try:
        mycursor.execute(
            "Update Book_BookStore set amount = amount - '%s' where ISBN = '%s'" % (amount, isbn))
        mydb.commit()
...
def addNewOrder(id, oNum, tNum, ba, sa, month, year):
    # get month and year
    try:
        mycursor.execute("insert into Orders values ('%s','%s','%s','%s','%s',%d,%d)" % (
            id, oNum, tNum, ba, sa, month, year))
        mydb.commit()
...
def addNewBook_Order(id, oId, isbn, amount):
    try:
        mycursor.execute(
            "insert into Book_Orders values('%s','%s','%s',%d);" % (id, oId, isbn, amount))
        mydb.commit()
